...
// TODO(sumeer): compute a metric that tracks locality of values for keys in
// the same ssblock (are they in different value blocks). And an aggregation
// of this for the whole sstable. If the input sstables to a compaction have
// poor value locality, rewrite to new blob files.
...
// <end_of_file>
//
// There must be at least one value in a blob file.
// The value blocks are identical to the value blocks in sstables.
...
// - optimize valueHandle to return start offset in file for the block, which
//   is known (unless we start doing parallel compression). Then we don't need
//   to write blockOffset in the value blocks index.
...
// blobFileReaderInterface is a wrapper around blobFileReader. A cache
// provided implementation would wrap Close() and only call
// blobFileReader.Close() when the reader has been evicted from the cache and
// the last reference is calling Close(). A trivial implementation can simply
// create a new blobFileReader and return it.
...
// ProviderOfReaderForBlobFiles can return a blobFileReaderInterface for
// any blob FileNum. It is thread-safe. This is expected to be implemented
// by a cache of blobFileReaders.
...
// BlobFileReaderCache is a LRU cache.
...
// the iterator for reading from value blocks. It implements the ValueFetcher
// interface, and can fetch from any blob file. The reason it is scoped to the
// lifetime of a sstable iterator is that calling close() on this transitions
// it to a less efficient mode for fetching (see the memory management
// discussion in lazy_value.go and P2).
type blobValueReader struct {
	provider ProviderOfReaderForBlobFiles
	stats    *base.InternalIteratorStats

	fetchCount uint64
	// Up to 3 valueBlockReaders are cached. We expect the fanout from a single
	// sstable to blob files to be low. Caching is especially beneficial when
	// the corresponding sstable iterator has not closed the blobValueReader
	// since we can return values from Fetch that are backed by the
	// corresponding block, and the valueBlockReaders retain the latest
	// decompressed cache block for efficient reads when there is locality of
	// access. Once the blobValueReader is closed, the cached valueBlockReaders
	// are also in closed state and the benefit of this cache is limited to
	// avoiding repeated memory allocations in constructing a valueBlockReader
	// for each Fetch.
...
type cachedValueBlockReader struct {
	// The cache key. > 0 for a valid cache entry.
	fileNum base.FileNum
	...
	// methods will not be used until after closed, we use the
