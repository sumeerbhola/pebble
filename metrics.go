...
type LevelMetrics struct {
	...
	// We cannot accurately compute the per-level write-amp since we don't have
	// a BlobBytesIn metric representing the bytes coming in from a higher
	// level. It is not easy to measure compressed blob bytes in, since parts of
	// the blob file may be read. The per-level write amp will be significantly
	// higher than the actual due to this since the denominator in the write amp
	// calculation will not include the blob bytes.

	// BlobBytesCompacted is the size of the new blob files written to this
	// level as part of compaction.
	BlobBytesCompacted uint64
	// BlobBytesMoves is the size of the blob files moved from a higher level
	// into this level.
	BlobBytesMoved uint64
...
func (m *LevelMetrics) Add(u *LevelMetrics) {
	...
	m.BlobBytesMoved += u.BlobBytesMoved
...
func (m *LevelMetrics) format(w redact.SafePrinter, score redact.SafeValue) {
	...
	w.Printf("%5d(%5d) %6s(%6s %.2f) %7s %7s %7s %7s %5s(%5s) %7s %6s(%6s) %7s %7s %7d %7.1f\n",
		...
		humanize.IEC.Uint64(m.BlobBytesMoved),
...
func (m *Metrics) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	w.SafeString("__level________count________________size___score______in__ingest(sz_cnt)" +
		"______move(sz_sz_cnt)__________write(sz_cnt)____read___r-amp___w-amp\n")
	m.formatWAL(w)
	for level := 0; level < numLevels; level++ {
		l := &m.Levels[level]
