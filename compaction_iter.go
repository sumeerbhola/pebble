...
// The only place where the value will be actually lazy is when it is pointing
// to a blob file. The caller can check whether it wants to keep it as a
// pointer or not. If yes, it needs to reconstruct the handle since we have
// taken out the prefix, len and long attribute. Or somehow peer into the raw
// value. Can't peer into the raw value since need to punch through the
// iterator tree.
