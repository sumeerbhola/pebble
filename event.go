...
type CompactionInfo struct {
	...
	NotReusedDueToDepthBlobSize uint64
	BlobDecisions []BlobFileDecisionForCompaction
...
func (i CompactionInfo) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	if !i.Done {
		...
		if len(i.BlobDecisions) > 0 {
			/*
			w.Printf(" blobs ")
			for _, d := range i.BlobDecisions {
				if d.FileAtLowestCompactionLevel {
					w.Printf(" %t (l): refs: %d/%d live %.2f", d.ReuseReference, d.CompactionInputRefs, d.Refs, d.LiveFraction)
				} else {
					w.Printf(" %t: refs: %d/%d live %.2f", d.ReuseReference, d.CompactionInputRefs, d.Refs, d.LiveFraction)
				}
			}
			 */
			w.Printf(" blob sizes: input (lowest): %s, %s, reused (lowest): %s, %s, not reused: %s ref, %s g, %s depth",
				humanize.IEC.Uint64(i.InputBlobSize), humanize.IEC.Uint64(i.LowestLevelBlobSize),
				humanize.IEC.Uint64(i.ReusedBlobSize), humanize.IEC.Uint64(i.LowestLevelReusedBlobSize),
				humanize.IEC.Uint64(i.NotReusedDueToReferenceBlobSize), humanize.IEC.Uint64(i.NotReusedDueToGarbageBlobSize),
				humanize.IEC.Uint64(i.NotReusedDueToDepthBlobSize))
		}
		return
	}
...
type BlobFileCreateInfo struct {
	JobID int
	// Reason is the reason for the blob file creation: "compacting", "flushing", or
	// "ingesting".
	Reason  string
	Path    string
	FileNum FileNum
...
func (i BlobFileCreateInfo) SafeFormat(w redact.SafePrinter, _ rune) {
	w.Printf("[JOB %d] %s: blobf created %s",
		redact.Safe(i.JobID), redact.Safe(i.Reason), redact.Safe(i.FileNum))
}
...
func (i BlobFileDeleteInfo) SafeFormat(w redact.SafePrinter, _ rune) {
	if i.Err != nil {
		w.Printf("[JOB %d] blobf delete error %s: %s",
			redact.Safe(i.JobID), redact.Safe(i.FileNum), i.Err)
		return
	}
	w.Printf("[JOB %d] blobf deleted %s", redact.Safe(i.JobID), redact.Safe(i.FileNum))
}
...
type EventListener struct {
	...
	// BlobFileCreated is invoked when a blob file has been created.
